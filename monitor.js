...
function drawBubbles(d, r) {
    ...
    setInterval(function() {
        d3.json(nodesListURL, function(d) {
            updateBubbles(bubble, vis, d);
        });
    }, refreshInterval);
}
...
function monitorInit() {
    ...
    d3.json(nodesListURL, drawBubbles);
    setInterval(function() {
...
function detectCBFS(f) {
    $.ajax(nodesListURL, {
        error: function() {
            console.log("Not on CBFS, going offline");
            nodesListURL = "nodes-offline.json";
            allMediaURL = "media-offline.json";
        },
        success: function() {
            console.log("On CBFS, proceeding normally.");
        },
        complete: f
    });
}
